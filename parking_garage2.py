...
- If the payment variable is not empty then (meaning the ticket has been paid) 
   -> display a message to the user that theibuyr ticket has been paid and they have 15mins to leave
...
class Parking():
    tickets = ["tickets"] * 10
    parkingSpaces = ["spaces"] * 10
    currentTicket = {"paid": False}

    def takeTicket(self):
        Parking.tickets.pop()
        Parking.parkingSpaces.pop()
        print('Tickets Left {}'.format(len(Parking.tickets)))
        print('Spaces Left {}'.format(len(Parking.parkingSpaces)))
